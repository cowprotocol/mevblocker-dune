...
routes.get("/", (req, res) => {
  log.info("Health check requested", createLogContext({
    endpoint: "/",
    method: "GET",
...
routes.post("/", async (req, res) => {
  ...
  try {
    ...
    if (!req.body || typeof req.body !== 'object') {
      log.warn("Invalid request body", createLogContext({
        requestId,
    ...
    if (!request || typeof request !== 'object') {
      log.warn("Invalid JSON-RPC request structure", createLogContext({
        requestId,
    ...
    log.info("Bundle request received", createLogContext({
      requestId,
    ...
    if (request.method != "eth_sendBundle") {
      log.warn("Unsupported method requested", createLogContext({
        requestId,
        method: request?.method,
    ...
    if (!request.params) {
      log.warn("Missing request parameters", createLogContext({
        requestId,
        hasParams: 'params' in request,
    ...
    if (!Array.isArray(request.params)) {
      log.warn("Invalid request parameters type", createLogContext({
        requestId,
    ...
    if (request.params.length !== 1) {
      log.warn("Invalid request parameters length", createLogContext({
        requestId,
    ...
    if (!bundle || typeof bundle !== 'object') {
      log.warn("Invalid bundle object", createLogContext({
        requestId,
    ...
    if (!bundle.txs) {
      log.warn("Missing bundle transactions", createLogContext({
        requestId,
    ...
    if (!bundle.blockNumber) {
      log.warn("Missing bundle block number", createLogContext({
        requestId,
        hasBlockNumber: 'blockNumber' in bundle,
    ...
    if (!Array.isArray(bundle.txs)) {
      log.warn("Invalid bundle transactions type", createLogContext({
        requestId,
    ...
    if (bundle.txs.length === 0) {
      log.warn("Empty bundle transactions", createLogContext({
        requestId,
    ...
    const invalidTxs = bundle.txs.filter(tx => typeof tx !== 'string' || !tx.startsWith('0x'));
    if (invalidTxs.length > 0) {
      log.warn("Invalid transaction format", createLogContext({
        requestId,
      ...
      res.status(400).json({ 
        ...
        message: "All transactions must be valid hex strings starting with '0x'",
    ...
    if (txCount > 50000) {
      ...
      log.warn("Bundle exceeds transaction limit", createLogContext({
        requestId,
        bundleId: `${Number(bundle.blockNumber)}_${request.id}`,
        txCount,
        maxTxs: 50000
    ...
    if (estimatedMemoryMB > maxMemoryMB) {
      log.warn("Bundle exceeds memory limit", createLogContext({
        requestId,
        bundleId: `${Number(bundle.blockNumber)}_${request.id}`,
        txCount,
        estimatedMemoryMB,
    ...
    if (!Number.isFinite(blockNumberNum) || blockNumberNum < 0) {
      log.warn("Invalid block number", createLogContext({
        requestId,
    ...
    log.info("Bundle validated successfully", createLogContext({
      requestId,
      bundleId,
      blockNumber: bundle.blockNumber,
    ...
    if (activeBundleProcessing > 100) {
      log.warn("High concurrent bundle processing", createLogContext({
        requestId,
        bundleId,
    ...
    if (activeBundleProcessing > 500) {
      log.error("Extreme concurrent bundle processing - rejecting request", createLogContext({
        requestId,
        bundleId,
        activeBundleProcessing,
        threshold: 500,
    ...
    log.info("Bundle request processed successfully", createLogContext({
      requestId,
      bundleId,
      responseTime,
      referrer
    ...
    setImmediate(async () => {
      ...
      try {
        log.debug("Starting bundle conversion", createLogContext({
          requestId,
          bundleId,
          txCount: bundle.txs.length
        }));
        ...
        log.info("Bundle conversion completed", createLogContext({
          requestId,
          bundleId,
          conversionTime,
        ...
        setTimeout(async () => {
          ...
          try {
            log.info("Starting bundle upload", createLogContext({
              requestId,
              bundleId,
            ...
            log.info("Bundle upload completed successfully", createLogContext({
              requestId,
              bundleId,
              uploadTime,
              totalProcessingTime,
            ...
            const uploadTime = Date.now() - uploadStartTime;
            log.error("Bundle upload failed", createLogContext({
              requestId,
              bundleId,
              uploadTime,
    ...
    log.error("Request processing failed", createLogContext({
      requestId,
      totalTime,
      error: e instanceof Error ? e.message : String(e),
      stack: e instanceof Error ? e.stack : undefined
